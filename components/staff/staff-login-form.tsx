...
import { signInWithEmail } from '@/lib/actions/auth/staff';
...
export function StaffLoginForm({ error: urlError }: StaffLoginFormProps) {
    ...
    <div>
      {error && (
        <div className="mb-5 rounded-xl bg-red-50/90 border border-red-200/80 p-4 shadow-sm">
          ...
          {error.includes('Staff privileges required') && (
            <p className="mt-2 text-xs text-red-700">
              Only staff members (managers, sub-admins, account managers, employees) can access this portal.
            </p>
          )}
        </div>
      )}
